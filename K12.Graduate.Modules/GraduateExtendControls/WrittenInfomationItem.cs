...
namespace K12.Graduation.Modules
{
    //本資料項目用以儲存使用者設計之書面資料
    //如學籍表...
    [FISCA.Permission.FeatureCode("K12.Graduation.Modules.WrittenInfomationItem", "書面檔案")]
    public partial class WrittenInfomationItem : DetailContentBase
    {
        //權限
        ...
        void BGW_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
        {
            ...
            if (e.Error == null)
            {
                List<WrittenInformationUDT> listUDT = (List<WrittenInformationUDT>)e.Result;
                if (listUDT != null)
                {
        ...
        private void BindData(List<WrittenInformationUDT> listUDT)
        {
            foreach (WrittenInformationUDT eachUDT in listUDT)
            {
        ...
        {
            DialogResult dr = MsgBox.Show("您確定於封存資料中,刪除選擇之書面記錄?", MessageBoxButtons.YesNo, MessageBoxDefaultButton.Button2);
            ...
            {
                //說明:
                //CurrentRow的TAG內存之UID / 是WrittenInformationUDT的UID
                //欲取得GraduateUDT資料,需要先取得WrittenInformationUDT
                //後由WrittenInformationUDT的RefUDT_ID取得GraduateUDT資料
                ...
                List<WrittenInformationUDT> listUDT2 = _AccessHelper.Select<WrittenInformationUDT>(UDT_S.PopOneCondition("UID", uidList));
                if (listUDT2.Count > 0)
                {
                    ...
                    foreach (WrittenInformationUDT each in listUDT2)
                    {
                        sb.AppendLine("已刪除書面資料「" + each.Name + each.Format + "」");
                    }


                    _AccessHelper.DeletedValues(listUDT2.ToArray());

                    ApplicationLog.Log("畢業生檔案檢索.書面資料", "刪除", sb.ToString());
                    MsgBox.Show("已刪除所選書面資料");
                    e.Cancel = true;
        ...
        void linkBGW_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            string TableName = Tn._WrittenInformationUDT;
            ...
            else
            {
                ...
                foreach (string each in ofd)
                {
                    ...
                    Logsb.Append("書面名稱「" + udt.Name + udt.Format + "」");
                ...
                if (InsertUDTData.Count != 0)
                {
                    ...
                    ApplicationLog.Log("畢業生檔案檢索.個人書面資料", "批次上傳", Logsb.ToString());
                }
            }
        }
